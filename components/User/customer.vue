<template>
  ...
  >
    ...
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>مشتریان</v-toolbar-title>
                ...
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <v-text-field
                      ...
                      required
                      @input="$v.editedItem.nationalCode.$touch()"
                      @blur="$v.editedItem.nationalCode.$touch()"
                      v-model:trim="$v.editedItem.nationalCode.$model"
                    ></v-text-field>
                  </v-col>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      nationalCodeErrors() {
        const errors = [];
        if (!this.$v.editedItem.nationalCode.$dirty) return errors;

        !this.$v.editedItem.nationalCode.required && errors.push('کد ملی الزامی است');
        !this.$v.editedItem.nationalCode.numeric && errors.push('کارکتر غیر مجاز است');
        return errors
      },
      formTitle() {
